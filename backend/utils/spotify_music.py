...
# Load .env from project root — works from any subfolder
...
def init_spotify():
    global sp, SPOTIFY_AVAILABLE
    try:
        if not CLIENT_ID or not CLIENT_SECRET:
            print("WARNING: Spotify credentials missing.")
            return False
...
EMOTION_SEARCH = {
    ...
    'Sad':       'sad emotional heartbreak ballad',
    'Angry':     'angry aggressive rock metal',
    'Neutral':   'chill relaxing indie pop',
    'Surprised': 'energetic exciting upbeat',
    'Fearful':   'dark moody atmospheric',
    'Disgusted': 'aggressive hard rock punk',
}
...
FALLBACK_DB = {
    'Happy': [
        ...
        {'title':'Happy',                'artist':'Pharrell Williams', 'duration':'3:53','preview_url':None,'image':None,'spotify_url':'https://open.spotify.com/search/Happy%20Pharrell',                  'match':'97%'},
        ...
        {'title':'Shake It Off',         'artist':'Taylor Swift',      'duration':'3:39','preview_url':None,'image':None,'spotify_url':'https://open.spotify.com/search/Shake%20It%20Off',                  'match':'91%'},
...
def get_spotify_tracks(emotion, limit=5):
    ...
    if not SPOTIFY_AVAILABLE or sp is None:
        return None
    try:
        query   = EMOTION_SEARCH.get(emotion, 'popular music')
        ...
        for item in results['tracks']['items']:
            ...
            tracks.append({
                ...
                'match':       f"{95 - len(tracks)*3}%",
                'id':          item['id']
            })
        print(f"Spotify returned {len(tracks)} tracks for {emotion}")
...
def get_recommendations_for_emotion(emotion, limit=5):
    ...
    tracks  = get_spotify_tracks(emotion, limit)
    if tracks:
        return tracks
    print(f"Using fallback tracks for {emotion}")
    return FALLBACK_DB.get(emotion, FALLBACK_DB['Neutral'])[:limit]
